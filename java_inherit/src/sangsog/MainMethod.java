...
//		Microwave microwave = new Microwave();
		...
		//서도 다른 이름을 가진 변수 여러개를 하나의 이름으로 묶어주기
		int[] arr = new int[5];
		int[] arr2= new int[6];
		int[] arr3= new int[7];
		int[][] temp1 = new int[3][];
		...
		// 같은 타일의 1차원 배열 여러개 라면 2차원 배열 하나로 묶어주기
		...
		// 클래스 가 여러개 필요하다 면 데이터를 중점으로 다루는 클래스가 있고,메서드를 중점으로 다루는 클래스가 있으니
		// 데이터관련 클래스를 여러개는 공통점이 있는 경우 부모 클랫로 묶어 준다.
		// 메서드를 중점으로 다루는 클래스가 여러개면 추상 부모클래스 또는 인터페이스로 묶어준다.
		
		
		Aircon air1 = new Aircon("캐리어",200);
		Television tv1 = new Television();
		tv1.ch=11;
		//Applince app1 = new Appliance(); //추상클래스인 부모 클래스는 객체생성 불가
		Appliance app1;
		app1 = air;
...
//추상 클래스 만드는 이유
// 상속관계에서 공통적인 부분을 부모클래스에 두는데 자식 클래스들 마다 특정 적인 동작을 위해 
// 추상메서드가 만들어 두어야 한다.

// 자식클래스들에게 규칙을 강제화 하기위해
// 설계도를 만들기 위해 
// 다향성을 안전하게 쓰기위해
