...
// 지금의 방식은 누구를 입력안했다고라고 표현하는게 아니고
// 모두 입력했냐 안했냐만 확인
function joinCheck(){
    let id= document.getElementById("userId");
    let pw= document.getElementById("userPw");
    ...
    if( count ==5){  //필수 이비력 모두 했다.
        if(pw.value != pw2.value){ //비밀번호와 비밀번호확인이 다른값 
            alert("비밀번호를 다시 확인해주세요");  // 자바 스크립트에서는 완전 같을때 === 세번사용 / ==만 사용시 10=="10" 도 같다고 나온다
            pw.value=''; //비밀번호 input값 비우기
            pw2.value=''; //비밀번호 확인 input값 비우기
            return;
        }
            document.getElementById("joinBt").disabled = false;
    }else { //필수입력중에 하나 이상 입력안함
